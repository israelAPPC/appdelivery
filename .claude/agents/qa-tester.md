...
hooks:
  PreToolUse:
    - matcher: Bash
      hooks:
        - type: command
          command: node .claude/hooks/qa/only-test-commands.js
---
...
- Nunca rodar comandos fora de execução de testes (o hook `only-test-commands.js` bloqueia isso automaticamente)
